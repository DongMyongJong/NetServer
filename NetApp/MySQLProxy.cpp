...
MySQLProxy::MySQLProxy(const char *schema, const char *ip, int port, const char *user, const char *password)
{
	_mysql = mysql_init((MYSQL*)NULL);
	if (_mysql == NULL)
	{
		printf("Failed init MySQL!");
	}
	if (!mysql_real_connect(_mysql, ip, user, password, schema, port, NULL, 0)) {
		printf("Failed connect MySQL Server!\n");
	}
	if (mysql_query(_mysql, "select * from fa_trans;") != 0) {
